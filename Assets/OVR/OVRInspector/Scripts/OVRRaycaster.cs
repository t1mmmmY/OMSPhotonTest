...
public class OVRRaycaster : GraphicRaycaster, IPointerEnterHandler, IPointerExitHandler
{
    //private MenuControl _menuControl; // DD DI
    ...
    //public Action<GameObject> OnBlockObjectEnter;
    //public Action<GameObject> OnBlockObjectExit;
    ...
    private void Raycast(PointerEventData eventData, List<RaycastResult> resultAppendList, Ray ray, bool checkForBlocking)
    {
        ...
        UnityEngine.RaycastHit[] hits3d = null;
        ...
        if (checkForBlocking && blockingObjects != BlockingObjects.None)
        {
            ...
            {
                hits3d = Physics.RaycastAll(ray, dist, m_BlockingMask);

                if (hits3d.Length > 0 && hits3d[0].distance < hitDistance)
                {
                    hitDistance = hits3d[0].distance;
                }
            }

            if (blockingObjects == BlockingObjects.TwoD || blockingObjects == BlockingObjects.All)
            {
                hits2d = Physics2D.GetRayIntersectionAll(ray, dist, m_BlockingMask);

                if (hits2d.Length > 0 && hits2d[0].fraction * dist < hitDistance)
                {
                    hitDistance = hits2d[0].fraction * dist;
                }
            }
        }

        // DD, if what we are (or no longer) looking at is of interest, then issue actions
 /*       if( _menuControl != null )
        {
            if( hits3d != null && hits3d.Length > 0 && IsFocussed() == false )
            {
                foreach( UnityEngine.RaycastHit hit in hits3d )
                {
                    if( blockingObject != hit.transform.gameObject )
                    {
                        // Was looking at something, and instently look at something else of interest
                        if( blockingObject != null )
                            _menuControl.LocationHoverExit(blockingObject);

                        blockingObject = hit.transform.gameObject;
                        if( blockingObject != null )
                            _menuControl.LocationHoverEnter(blockingObject);
                    }
                }
            }
...
*/
    ...
    //public void SetMenuControl(MenuControl menuControl_)
    //{
    //    _menuControl = menuControl_;
    //}
